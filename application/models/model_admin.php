...
class model_admin extends CI_Model {
	...
	public function add_temp_user($key) {
		...
		$query = $this -> db -> insert("temp_admin", $data);
		if ($query) {
	...
	public function add_user($key) {
		...
		$this -> db -> where("key", $key);
		$queryTemp_user = $this -> db -> get("temp_admin");
		...
		If ($queryTemp_user) {
			...
			$did_add_user = $this -> db -> insert("admin", $data);
		}

		if ($did_add_user) {

			$this -> db -> where("key", $key);
			$this -> db -> delete("temp_admin");
